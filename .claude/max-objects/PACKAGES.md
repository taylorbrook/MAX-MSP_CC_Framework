...
- **Bach llll**: Bach, Cage, Dada, EARS all use the llll (Lisp-like linked list) data type. lllls are NOT compatible with standard MAX lists. Never connect a regular MAX list outlet to a bach object expecting llll input. The dedicated list/llll converter objects named in older docs are **not present in the installed bach package** (they are marked `verified_installed: false` and the package critic emits a blocker for them). Instead: a flat MAX list connects directly to a bach inlet; use `bach.flat`, `bach.iter`, or `bach.pack` for structured conversion, and `bach.nth N` (1-based, `@unwrap 1` when the element is itself a sublist) for extraction.
