...
5. **Route through max-router** -- invoke the max-router skill with:
   - The user's task description
   - Project context
   - Relevant memory entries
   - The router analyzes keywords/intent and dispatches to specialist agent(s)

6. **Specialist generation** -- the router dispatches to one or more specialist agents:
...
10. **Update status** -- set stage to "build" and increment progress via `update_status()`.
