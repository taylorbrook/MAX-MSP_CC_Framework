...
## v0.2.4 (2026-08-23) — gate loudness weighting for sustained fricative noise

User report: held "sh"-type sounds still read very quiet. Root cause is level-vs-loudness: fricative energy sits at 2-8 kHz where the ear is most sensitive, so it is perceptually loud at genuinely low dBFS — a flat peak detector reads it "correctly" but uselessly. Added an HF-weighted sidechain branch: onepole HP at ~1.2 kHz (same DC-blocker form as the 70 Hz stages, coeff exp(-2pi*1200/sr)), instant-attack/300 ms-release peak envelope on that band, weighted x4 (+12 dB); gate takes it via max. Held noise and hiss now register near played-note levels; low/mid pitched material unaffected (440 Hz sine picks up ~+3 dB via this branch; high tones read hot, which is fine for a silence gate). Note: room hiss also reads ~+9-12 dB hotter, so floor settings need to come up correspondingly — relative margin between room tone and performed noise is unchanged. New Histories: hbx, hby, pkhf. Also applied to both embedded copies in 0_Burnt.maxpat.
