...
- v0.2.1 (2026-08-25): gen~ failed to compile ("use of undeclared identifier 'sr'" / `maxb_NN`). Cause: gen~ hoists Param-only expressions (e.g. `D = max(1, floor(delay_ms * 0.001 * sr))`) out of the per-sample body, where loop-local aliases (`sr = samplerate`, `RING = 1344000`) are not declared. Fix: use `samplerate` and literal constants directly in any expression whose inputs are only Params/constants. Also replaced `if (sh == N)` with range tests (gen~ warns on `==` in if).
- v0.2.2 (2026-08-25): grain read positions are stored in `Data` (float32); absolute sample counts lose the ±1 step after 2^24 samples (~6 min @ 48k). Now stored ring-wrapped (`wrap(x, 0, 1344000)`) at spawn and per step. Perception note: reversal is per-grain — at Grain 200 ms the stream reads forward with backwards attacks; raise Grain (500–2000 ms) and lower Density for obvious reversal (verified by chirp simulation).
- v0.3.0 (2026-08-29): added `reverse-delay-mono.maxpat` — mono-in / stereo-out variant of the same engine. `adc~ 1` → single input `gain~`/`meter~` → gen~ (codebox now 1 signal inlet: `inr = in1`; `source_mode` Param, its spawn branch, the Source umenu/message/label and their preset entries removed). Ring, feedback path, width panning and stereo output are unchanged. Original stereo patch untouched. Note: `update_version_comment` does not match the `REVERSE DELAY  vX.Y.Z` title (regex is anchored) and adds a stray comment — set the title text directly; `repair_text_contrast` also flipped the title to dark on this patch (reverted), so diff textcolors after calling it.
- v0.3.2 (2026-09-16): a friend's Max 9 build failed to compile the codebox with `use of undeclared identifier 'maxb_64'`…`'maxb_69'` (six consecutive generated lines = the six `max()` size guards `D`, `G`, `gmin`, `gmax`, `maxlat`, `mindel`), while the same code compiled on Max 9.1.5 here. `maxb_NN` is gen~'s temp for the second `max()` argument; when the expression is Param/constant-only it gets hoisted and on that build the temp lands in the wrong scope. Fix: every Param-only `max(a, b)` (the six guards plus `interval`, the four allpass lengths, the epsilon guards in `shapenorm`/`tiltnorm`/`looptrim`/`fwdtrim`, and the `max(dfloor, maxlat)` clamp bound) is now the exact identity `0.5 * (a + b + abs(a - b))` — pure arithmetic, no temp. Per-sample `max()` calls (those seeing `noise()` or grain state) are untouched. Same fix applied to the deployed copy in `Burnt-PerformancePackage/.../0_Burnt.maxpat` (both `p revdelay1` / `p revdelay2` codeboxes). Rule: in codebox, avoid `max()`/`min()` in Param-only expressions — use the abs identity.
- v0.3.3 (2026-09-16): v0.3.2's abs identity failed on Max 9.1.5 here with `use of undeclared identifier 'gmax_raw'` at generated line 2827 while `gmin_raw` was declared two lines earlier — i.e. the same defect as the friend's `maxb_NN`: gen~ hoists Param/constant-only statements out of the sample loop and emits them out of dependency order (which statements break varies by build). Fix: stop hoisting entirely. `History one(1)` (never written, always 1.0) is runtime state; `srs = samplerate * one` and `k_<param> = <param> * one` are computed once per sample at the top and every downstream expression uses those, so nothing in the codebox is Param-only any more. All `max()` calls restored to their original form. ~150 extra flops/sample (vs ~1500 in the 32-grain loop). `feedback`/`lowcut`/`highcut` keep their History smoothers (already per-sample). Applied to both repo variants and the two codeboxes in `0_Burnt.maxpat`. Confirmed compiling and sounding right on Max 9.1.5 (2026-09-16); friend's Max 9 build pending.
