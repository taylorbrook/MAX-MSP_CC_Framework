...
- Box matching (v0.8.1): mixer-manager.js finds strips/busses by varname (`getnamed`), never by maxclass -- `maxclass === "bpatcher"` never matched in MAX and every load stacked a duplicate `[N]` set (found with 35 bpatchers in the file). bpatcher rects equal their panel sizes; GAP 12 keeps the 92 px pitch.
