...
- Solo (v0.6.0): solo-in-place. Strips `send mixer-solo` -> `solo N 0/1` -> mixer-manager.js -> `messnamed mixer-solo-any`. Busses are solo-safe. Solo only gates the strip output; sends still follow their tap points (review item 9, open).
