...
- **No `max()`/`min()` in Param-only expressions either.** gen~ emits the second argument as a temp (`maxb_NN`/`minb_NN`); when the expression is hoisted, some Max 9 builds declare that temp in the wrong scope and fail with `use of undeclared identifier 'maxb_NN'` — even though the same code compiles on other 9.x builds (reverse-delay v0.3.2: compiled on 9.1.5 here, failed on a friend's Max 9). Write the exact identity instead: `max(a,b)` = `0.5 * (a + b + abs(a - b))`, `min(a,b)` = `0.5 * (a + b - abs(a - b))`. Per-sample `max()`/`min()` (inputs include signals, `noise()`, History/Data reads) are fine.
